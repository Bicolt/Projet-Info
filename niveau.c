 ...
 *
 * \details   	Permet de charger un niveau en particulier et gÃ¨re son dÃ©filement
 * \details		ainsi que les diffÃ©rentes intÃ©ctions entre le joueur et le terrain
 */
...
/**
 * \param H_PERSO dÃ©finit la hauteur du personnage Ã  l'Ã©cran
 * \param L_PERSO dÃ©finit la largeur du personnage Ã  l'Ã©cran
 * \param EPAISSEUR_TRAIT dÃ©finit l'Ã©paisseur par dÃ©faut des traits tracÃ©s Ã  l'Ã©cran
 * \param X_FIN dÃ©finit l'abscisse de fin de niveau
 * \param NOMBRE_ECRANS dÃ©finit la largeur, en nombre d'Ã©crans, de chaque niveau
 */
 ...
 *
 *	Cette fonction gÃ¨re l'affichage d'un niveau
 *	ainsi que les intÃ©ractions utilisateurs avec ce dernier
 *	\param ecran La surface visible Ã  l'Ã©cran sur laquelle est affichÃ© la partie courante du niveau
 *	\param choixTerrain Le niveau sÃ©lectionnÃ© dans le menu de sÃ©lection du niveau
 *	\return Un \e int correspondant aux diffÃ©rentes fins possibles du niveaux (victoire, mort, ...)
 */
int niveau(SDL_Surface *ecran, int choixTerrain){
    ...
    int xRinit = 0, yRinit = 0; //dÃ©finitions de diffÃ©rentes positions utilisÃ©es pour les calcul d'angle, de déplacement, ...
